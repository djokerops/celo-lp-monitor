...
async function main() {
  ...
  for (const [addr, meta] of universe) {
    ...
    if (!d) {
      ...
      row.flags.push({ type: "data_unavailable", severity: "info", notify: false, detail: "no Dexscreener pair for this pool" });
      pools.push(row);
      continue;
    }

    const tvl = Number(d.liquidity?.usd) || 0;
    const price = Number(d.priceUsd) || 0;
    const baseAmt = Number(d.liquidity?.base) || 0;
    const baseSym = norm(d.baseToken?.symbol);
    const quoteSym = norm(d.quoteToken?.symbol);
    const baseUsd = baseAmt * price;
    const basePct = tvl > 0 ? Math.min(100, Math.max(0, (baseUsd / tvl) * 100)) : 0;
    const h24 = Number(d.priceChange?.h24) || 0;

    Object.assign(row, {
      tvlUsd: tvl, priceUsd: price, h24,
    ...
    if (USD_STABLE.has(baseSym) && USD_STABLE.has(quoteSym) && price > 0) {
    ...
    if (meta.rule !== "redeposit" && haveBaseline && baseSkew != null) {
    ...
    if ((VOLATILE.has(baseSym) || VOLATILE.has(quoteSym)) && Math.abs(h24) > VOL_H24_PCT)
    ...
    if (!samples.some(s => s.day === today)) samples.push({ day: today, tvlUsd: tvl, skewPct: basePct, internalUsd: meta.internalUsd });
  ...
  console.log(`pool health: ${pools.length} pools, ${report.flagged.length} flagged, ${pools.filter(p => p.unavailable).length} without data${errors.length ? `, ${errors.length} fetch error(s)` : ""}`);
}
